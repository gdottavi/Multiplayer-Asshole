...
class Deck extends Array {
    ...
    static serialize(deck) {
        ...
        if (deck.cards && Array.isArray(deck.cards)) {
            serializedCards = deck.cards.map((card) => card_1.Card.serialize(card));
        }
        return {
    ...
    static deserialize(serializedDeck) {
        ...
        if (serializedDeck.cards && Array.isArray(serializedDeck.cards)) {
            deck.cards = serializedDeck.cards.map((serializedCard) => card_1.Card.deserialize(serializedCard));
        }
        else {
