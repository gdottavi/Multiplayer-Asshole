...
class GameRuleHandler {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            cardsPlayed.forEach(cardPlayed => {
                ...
                if (socketId !== this.scene.socket.id) {
                    ...
                    (0, utils_1.removeSprite)(this.scene, cardPlayed);
    ...
    canPlay(cardsPlayed) {
        ...
        if (cardsPlayed.length > 4) {
            (0, utils_1.createToast)(this.scene, "Cannot play " + cardsPlayed.length + " cards at once.");
            return false;
        }
        //multiple cards played with different values
        if (!cardsPlayed.every(card => card.value === cardsPlayed[0].value)) {
            (0, utils_1.createToast)(this.scene, "Cannot play multiple cards of different values.");
            return false;
        }
        //first card played
        ...
        if (cardsPlayed.length < lastPlayedHand.length) {
            (0, utils_1.createToast)(this.scene, "Last play was " + lastPlayedHand.length + " of a kind.  You only played " + cardsPlayed.length + " cards.");
            return false;
        }
        //single card played
        if (cardsPlayed.length === 1) {
            if (cardsPlayed[0].rank < lastPlayedHand[0].rank) {
                (0, utils_1.createToast)(this.scene, "Must play doubles higher than last played double");
                return false;
            }
        }
        //doubles played
        if (cardsPlayed.length === 2) {
            if (cardsPlayed[0].value === two || cardsPlayed[0].value === four) {
                (0, utils_1.createToast)(this.scene, "Cannot play multiple 2s or 4s");
                return false;
            }
        ...
        if (cardsPlayed.length === 3) {
            ...
            if (lastPlayedHand.length < 3)
                return true;
            if (cardsPlayed[0].rank < lastPlayedHand[0].rank) {
                (0, utils_1.createToast)(this.scene, "Must play triples higher than last played triple");
                return false;
            }
        }
        //default
    ...
    checkSquareCompleted(cardsPlayed) {
        ...
        let playedCards = (0, utils_1.getAllPlayedCards)(this.currentPlayedCards);
        //first card played
        ...
        if (cardsPlayed.length === 2) {
            if (!(0, utils_1.areLastXValuesEqual)(playedCards, 2))
                return false;
            return true;
        }
        //1 card played on 3 matches
        if (cardsPlayed.length === 1) {
            if (!(0, utils_1.areLastXValuesEqual)(playedCards, 3))
                return false;
            return true;
        }
        //default
